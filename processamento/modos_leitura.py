...
NOMES = {
    "cor_melhorada": "Cor melhorada",
    "cinza": "Escala de cinza",
    "alto_contraste": "Alto contraste",
    "texto_pb": "Preto no branco",
    "texto_invertido": "Branco no preto",
    "amarelo_preto": "Amarelo no preto",
}
...
def aplicar_modo(frame, modo):
    ...
    if modo == "texto_pb":
        out = _texto_binario(frame)
        return cv2.cvtColor(out, cv2.COLOR_GRAY2BGR)

    if modo == "texto_invertido":
        out = cv2.bitwise_not(_texto_binario(frame))
        return cv2.cvtColor(out, cv2.COLOR_GRAY2BGR)
